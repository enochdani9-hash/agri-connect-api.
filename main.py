...
def get_me(token: str, db: Session = Depends(get_db)):
    ...
    trial_days_left = max(0, 30 - ((datetime.utcnow() - user.created_at).days if user.created_at else 0))
    return {"full_name": user.full_name, "is_verified": user.is_verified, "email": user.email, "phone_number": user.phone_number, "trial_days_left": trial_days_left}
...
# --- NEW: MASTER DIRECTORY GET ALL ADS ---
