...
app = FastAPI(title="AgriConnect Production API", version="2.13")
...
@app.get("/")
def home():
    return {"message": "AgriConnect Production API is live with Buyer Auth & Search Integration."}
...
async def create_product(
    ...
    try:
        ...
        return {"status": "Success", "message": "Harvest listed live!", "listing": insert_res.data}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
def get_my_listings(authorization: Optional[str] = Header(None)):
    ...
    try:
        ...
        return {"farmer": farmer_name, "total_my_listings": len(response.data), "results": response.data}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
def delete_product(product_id: int, authorization: Optional[str] = Header(None)):
    ...
    try:
        ...
        return {"status": "Success", "message": f"Product ID {product_id} removed."}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
